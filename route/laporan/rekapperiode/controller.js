...
define([
    ...
    return ['$scope', '$routeParams', "$export", '$log', '$button', '$auth', '$popup', 'Iconplus_Master_Wilayah',
        ...
        var tableFilter = {};
            ...
            $scope.$watchGroup(["cari.kode","cari.tahun","cari.tahun2"], function (val) {
                if(val) {
                    $scope.table.reload();
                }
            });
            ...
            $scope.table = {
                ...
                reload: function () {
                    ...
                    param.select = "KODE_WILAYAH, NAMA_WILAYAH, NOURUT, KODE, KODENO, INDIKATOR, SUM(PENGHEMATAN_SD) PENGHEMATAN_SD, MAX(TARGET) TARGET, REGIONAL";
                    param.where  = "thbllap between "+moment($scope.cari.tahun).format('YYYYMM')+" and "+moment($scope.cari.tahun2).format('YYYYMM')+" and kode_wilayah = '"+$scope.cari.kode+"'";
                    param.group = "KODE_WILAYAH, NAMA_WILAYAH, NOURUT, KODE, KODENO, INDIKATOR, REGIONAL";
                    param.order = "nourut";
            ...
            $scope.reload = function(){
                var param = {};
                param.select = "KODE_WILAYAH, NAMA_WILAYAH, INDIKATOR, KETERANGAN, (case when keterangan = 'Target' then MAX(JUMLAH) ELSE SUM(JUMLAH) END) JUMLAH,  MIN(THBLLAP) THBLMIN, MAX(THBLLAP) THBLMAX";
                param.where  = "THBLLAP between "+moment($scope.cari.tahun).format('YYYYMM')+" and "+moment($scope.cari.tahun2).format('YYYYMM')+" and kode_wilayah = '"+$scope.cari.kode+"'";
                param.group = "KODE_WILAYAH, NAMA_WILAYAH, INDIKATOR, KETERANGAN";
                param.order = "KETERANGAN";
                param.kode_wilayah = $scope.cari.kode;
                VDashboard_Rekap.list(param).then(function(response){
                    for(var i = 1; i <= 2; i++){
