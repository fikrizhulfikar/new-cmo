    ...
    return ['$scope', '$timeout', '$routeParams', '$button', '$log', '$auth', '$notification',
        ...
        function($scope, $timeout, $routeParams, $button, $log, $auth,$notification,
            ...
            $scope.reload = function(){
                Master_Data_Rekon.list({select : "nama_unit, jenis_layanan, sum(jumlah) jumlah, sum(total) total",
                    where  : "THBLLAP = 201812 and unit_id = "+$auth.userdata.unitid,
                    group  : "NAMA_UNIT, JENIS_LAYANAN",
                    order : "NAMA_UNIT"}).then(function(response){
                ...
                Master_Data_Rekon.list({where  : "THBLLAP = 201812 and unit_id = "+$auth.userdata.unitid}).then(function(responsev){
