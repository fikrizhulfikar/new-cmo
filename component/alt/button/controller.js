define([
    ...
    return ['$scope', '$log', '$button', '$rootScope', '$element', function($scope, $log, $button, $rootScope, $element){
        ...
        $scope.dblclick = function(){
            if($scope.disabled || $scope.isdblclicked || $scope.href != '') return;

            var title = $scope.title;
            $scope.isdblclicked = true;
            $scope.title = $scope.title_clicked || title;

            var dblclick = $scope.ondblclick($scope);
            if(typeof dblclick !== 'undefined' && typeof dblclick.then === 'function'){
                dblclick.then(function(){
                    $scope.isdblclicked = false;
                    $scope.title = title;
                }, function(){
                    $scope.isdblclicked = false;
                    $scope.title = title;
        ...
        $element.attr('style', 'display:inline-block;' + $scope.style);
    }];
});