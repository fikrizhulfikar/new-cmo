define([
    ...
    return [
        ...
        'Iconplus_Master_Harga_Sewa','Iconplus_Master_Harga_Instalasi', 'Iconplus_Master_Permohonan','Iconplus_Master_Rekon',
        function($scope, $routeParams, $auth, $log, $button, $popup, $validate, $alert, $window,
                 ...
                 Iconplus_Master_Harga_Instalasi, Iconplus_Master_Permohonan,Iconplus_Master_Rekon){
            ...
            Iconplus_Master_Unit.retrieve({unitid:$scope.unitid}).then(function(response){
                ...
                $scope.regional = response.data.regional;
                console.log("tes"+$scope.unitid);
            });

            // Iconplus_Master_Permohonan.list().then(function(response){
            //     $scope.ref.permohonan  = response.data;
            // });  dipindahkan ke master rekon saja
            Iconplus_Master_Rekon.list({where : "thbllap = 201812 and unit_id = "+$auth.userdata.unitid}).then(function(response){
                $scope.ref.permohonan  = response.data;
            });
            ...
            $scope.$watch('modal.data.keteranganlayanan',function(val){
                if (val == 'clearchannelscada' || val == 'ipvpnscada') {
                    $scope.modal.data.kapasitas = '2 Mbps';
                };
            });
            ...
            $scope.pilih_unit = function (name,unitid){
                ...
                Iconplus_Master_Rekon.list({where : "thbllap = 201812 and unit_id = "+$scope.data.hidden_unitid}).then(function(response){
                    $scope.ref.permohonan  = response.data;
                });
            }
            ...
            $scope.pilih_serviceid = function (nomor_sid){
                ...
                Iconplus_Master_Permohonan.retrieve({where : "nomor_sid  like '%"+nomor_sid+"'"}).then(function(response){
                    $scope.modal.data.jenislayanan  = response.data.jenislayanan;
                    // $scope.modal.data  = response.data;
                    $scope.modal.data.nomorsid = response.data.nomor_sid;
                    $scope.modal.data.jenislayanan = response.data.jenislayanan;
                    $scope.modal.data.keteranganlayanan = response.data.keteranganlayanan;
                    $scope.modal.data.lokasi = response.data.lokasi;
                    $scope.modal.data.catatan = response.data.catatan;
                    $scope.modal.data.jarak = response.data.jarak;
                    $scope.modal.data.kapasitas = response.data.kapasitas;
                    $scope.modal.data.originating = response.data.originating;
                    $scope.modal.data.koordinat_originating = response.data.koord_originating;
                    $scope.modal.data.alamat_originating = response.data.alamat_originating;
                    $scope.modal.data.terminating = response.data.terminating;
                    $scope.modal.data.koordinat_terminating = response.data.koord_terminating;
                    $scope.modal.data.alamat_terminating = response.data.alamat_terminating;
                });
            }
            ...
            $scope.$watch('modal.data.jenislayanan', function (val) {
                if(val) {
                    if (val == 'vsat') {
                        Iconplus_Master_Harga_Instalasi.retrieve({
                            jenis_layanan: $scope.modal.data.jenislayanan, regional: $scope.regional
                        }).then(function (response) {
                    ...
                    if (val != 'vsat') {
                        Iconplus_Master_Harga_Instalasi.retrieve({
                            jenis_layanan: $scope.modal.data.jenislayanan
                        }).then(function (response) {
                            $scope.modal.data.rpinstalasi = response.data.rpinstalasi;
                        })
                    }
