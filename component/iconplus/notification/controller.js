define([
    ...
    return ['$scope', '$rootScope', '$routeParams', '$log', '$auth', 'Iconplus_Master_Notifikasi',
        function($scope, $rootScope, $routeParams, $log, $auth, Iconplus_Master_Notifikasi){
            $scope.notif = {};
            $scope.count = 0;
            ...
            $scope.isgetnotif = false;
            $scope.refreshNotifikasi = function () {
               // if (!$scope.isgetnotif) {
                    $scope.isgetnotif = true;
                    $scope.count = 0;
                    $scope.notif = {};
                    var unitid = $auth.userdata.unitid || '';
                    Iconplus_Master_Notifikasi.list({unitid: unitid}).then(function (response) {
                        $scope.isgetnotif = false;
                        angular.forEach(response.data, function (jenis, kj) {
                            if ($scope.setting.hasOwnProperty(kj)) {
                                if (!$scope.notif[kj]) $scope.notif[kj] = {};
                                angular.forEach(jenis, function (subjenis, ksj) {
                                    ...
                                    if ($scope.setting[kj].hasOwnProperty(key)) {
                                        if ($scope.setting[kj][key]['usergroups'].indexOf(
                                            ...
                                            $scope.notif[kj][key] = subjenis;
                                            $scope.count += parseInt(subjenis);
                                        }
                                    }
                                });
                            }
