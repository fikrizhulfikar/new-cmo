define([
    ...
    moment.locale('id');
    return ['$scope', '$routeParams', '$log', '$auth','Dashboard_Status','Iconplus_Rekap_Laporan',
        function($scope, $routeParams, $log, $auth, Dashboard_Status, Iconplus_Rekap_Laporan){
            $scope.moment = moment;
            ...
            window.dashboardRegional = function(tahun, regional, thblmin, thblmax){
                Iconplus_Rekap_Laporan.table({ select: "KODE_WILAYAH, NAMA_WILAYAH, NOURUT, KODE, KODENO, INDIKATOR, SUM(PENGHEMATAN_SD) PENGHEMATAN_SD, MAX(TARGET) TARGET, REGIONAL",
                    where:"regional = '"+regional+"' and thbllap between '"+$scope.cari.thblmin+"' and '"+$scope.cari.thblmax+"'",
                    group: "KODE_WILAYAH, NAMA_WILAYAH, NOURUT, KODE, KODENO, INDIKATOR, REGIONAL",
                    order: "nourut"}).then(function(response){
            ...
            $scope.init = function(){
                $scope.filter.where = "id_regional is not null";
                    $scope.rekap = angular.copy({});
                    angular.forEach($scope.data, function(item){
                        $scope.cari.thblmin = item.thblmin;
                        $scope.cari.thblmax = item.thblmax;
                        // console.log($scope.cari.thblmin);
                        var tahun = item.regional;
            ...
            $scope.reload = function(rekap){
                if(rekap) {
                    ...
                    angular.forEach(rekap, function(item, yyyymm){
                        category.push({
                            label: item.label,
                            thblmin:item.thblmin,
                            thblmax:item.thblmax
                        });
